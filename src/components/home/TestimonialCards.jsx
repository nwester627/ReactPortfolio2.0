...
export default function TestimonialCards() {
  ...
  const RoleAvatar = ({ person }) => {
    ...
    return (
      <div
        className="relative h-[70px] w-[70px] rounded-full shadow-lg ring-2 ring-white/20 dark:ring-white/10 shrink-0 flex items-center justify-center transition-all duration-300 hover:scale-105"
        style={{
        ...
        <div className="flex flex-wrap justify-center gap-2 text-sm font-medium">
          {testimonials.map((ti, i) => (
            <button
              key={ti.id}
              onClick={() => setIndex(i)}
              className={`px-4 py-2 rounded-full ring-1 transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lavender/60 font-semibold ${
                i === index
                  ? isDarkMode
